...
def get_file_list(fullpath_config, hla_info):
	...
	if str(hla_info).upper() == 'YES' or str(hla_info).upper() == 'Y' : 
		cmm = "bash /data/project/MOTIE/Tool/calcpkg/lib/neopepsee/pipe_neopepsee.sh {0} {1} {2} {3} {4} {5} {6} {7}".format(Ins[0],Ins[1],Ins[2],Ins[3],Ins[4],Ins[5],Ins[6], Ins[7])
	elif str(hla_info).upper() =='NO' or str(hla_info).upper() == 'N' :
		cmm = "bash /data/project/MOTIE/Tool/calcpkg/lib/neopepsee/pipe_optitype_neopepsee.sh {0} {1} {2} {3} {4} {5} {6} {7}".format(Ins[0],Ins[1],Ins[2],Ins[3],Ins[4],Ins[5],Ins[6],Ins[7])
	print(cmm)
	os.system(cmm)
